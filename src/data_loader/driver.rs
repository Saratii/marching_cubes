use crate::{
    ...
    terrain::{
        ...
        terrain_material::{TerrainMaterialExtension, TriData},
    },
};
use bevy::{
...
pub enum ChunkSpawnResult {
    ToSpawn(((i16, i16, i16), Mesh, Vec<TriData>)), //when a chunk is spawned without a collider
    ToSpawnWithCollider(((i16, i16, i16), Collider, Mesh, Vec<TriData>)), //when a chunk is spawned with a collider
    ToDespawn((i16, i16, i16)),
    ToGiveCollider(((i16, i16, i16), Collider)), //same lod but now needs a collider
    ToChangeLod(((i16, i16, i16), Mesh, Vec<TriData>)), //change mesh, assume it has no collider and doesnt need one
    ToChangeLodAddCollider(((i16, i16, i16), Mesh, Collider, Vec<TriData>)), //when its both changing LOD and now needs a collider
}
                            ...
                            Uniformity::NonUniform => {
                                let has_surface = match request.load_state_transition {
                                    LoadStateTransition::ToLod5 => {
                                        ...
                                        let has_surface = if chunk_contains_surface(
                                            &density_buffer_r5,
                                        ) {
                                            let (vertices, normals, tri_data, indices, tri_ids) =
                                                mc_mesh_generation(
                                                    &density_buffer_r5,
                                                    &material_buffer_r5,
                                                    RF5_SAMPLES_PER_CHUNK_DIM,
                                            ...
                                            let mesh = generate_bevy_mesh(
                                                vertices, normals, indices, tri_ids,
                                            );
                                            ...
                                            if had_entity_before {
                                                chunk_spawn_channel
                                                    .send(ChunkSpawnResult::ToChangeLodAddCollider(
                                                        (chunk_coord, mesh, collider, tri_data),
                                                    ))
                                                    .unwrap();
                                            } else {
                                                chunk_spawn_channel
...
pub fn chunk_spawn_reciever(
    ...
    while let Ok(req) = req_rx.0.try_recv() {
        ...
        count += 1;
        match req {
            ChunkSpawnResult::ToSpawn((chunk_coord, mesh, tri_data)) => {
            ...
            ChunkSpawnResult::ToChangeLodAddCollider((
                chunk_coord,
                new_mesh,
                new_collider,
                tri_data,
            ...
            ChunkSpawnResult::ToChangeLod((chunk_coord, new_mesh, tri_data)) => {
                let tri_data_bytes = bytemuck::cast_slice(&tri_data);
                let material_handle = materials.add(ExtendedMaterial {
                    base: StandardMaterial {
                        perceptual_roughness: 0.8,
            ...
            ChunkSpawnResult::ToSpawnWithCollider((chunk_coord, collider, mesh, tri_data)) => {
                let tri_data_bytes = bytemuck::cast_slice(&tri_data);
                let material_handle = materials.add(ExtendedMaterial {
                    base: StandardMaterial {
                        perceptual_roughness: 0.8,
